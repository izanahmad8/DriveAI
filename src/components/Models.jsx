...
export default function Models() {
  ...
  const filteredCars = useMemo(() => {
    let result = cars;
    if (modelFilter === 'suv') result = result.filter(c => c.type.toLowerCase().includes('suv'));
    if (modelFilter === 'ev') result = result.filter(c => c.type.toLowerCase().includes('ev'));
    if (modelFilter === 'under40') result = result.filter(c => c.price < 4000000);
            ...
            >
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="text-sm px-2 py-1 bg-white/10 rounded text-gray-300">{car.type}</span>
                </div>
                <p className="text-xl text-indigo-400 mb-4">{formatPrice(car.price, car.priceUSD, currency)}</p>
