...
export default function AIAssistant() {
  ...
  const [messages, setMessages] = useState([
    { role: 'assistant', text: "Hi! I'm DriveAI, powered by Gemini. How can I help you navigate your journey today? Try asking me to show SUVs, compare models, or book a test drive." }
  ]);
  const messagesEndRef = useRef(null);
  
  const { setCurrency, setModelFilter, setCompareModels, setBookingDetails, setHighlightedCarId } = useAppContext();
  ...
  const handleQuery = async (query) => {
    ...
    if (!apiKey) {
      setMessages(prev => [...prev, { role: 'assistant', text: "Please add your VITE_GEMINI_API_KEY to the .env.local file to use the AI Assistant." }]);
      return;
    }
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', text: `API Error: ${error.message || "Parsing failed. Please check console."}` }]);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-80 md:w-96 bg-gray-900 border border-white/10 rounded-2xl shadow-2xl z-50 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5">
          ...
          <div className="p-4 h-80 overflow-y-auto flex flex-col gap-3 bg-black/50">
            {messages.map((m, i) => (
              <div key={i} className={`max-w-[80%] p-3 rounded-2xl text-sm ${m.role === 'user' ? 'bg-indigo-500 text-white self-end rounded-tr-sm' : 'bg-white/10 text-gray-200 self-start rounded-tl-sm'}`}>
                {m.text}
              </div>
            ))}
            {isLoading && (
          ...
          <form onSubmit={handleSubmit} className="p-3 bg-gray-900 border-t border-white/10 flex items-center gap-2">
            ...
            <input 
              ...
              disabled={isLoading}
            />
            <button type="submit" className="p-2 bg-indigo-600 hover:bg-indigo-700 rounded-full text-white transition disabled:opacity-50" disabled={!input.trim() || isLoading}>
              <Send size={18} />
            </button>
          </form>
        </div>
